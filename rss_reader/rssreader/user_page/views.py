...
from django.shortcuts import render_to_response, HttpResponse, redirect, get_list_or_404
...
def delete_url(request):
    if request.method == 'POST':
        ...
        site_name = request.POST['site_name'].rstrip()

        site = Feed.objects.filter(user_id=user_id, feed_site_name=site_name)
        site.delete()

        return HttpResponse(json.dumps({'status': 'success'}, indent=2), content_type='application/json; charset=UTF-8')
